...
public class SandArtPanel extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
   ...
   public static void showBoard(Graphics g) {
      ...
      if (mouseX >= x)
         ...
         if (cursorMode == ERASE)
            g.drawImage(eraseMouse.getImage(), mouseX - (brushSize), mouseY - (brushSize), brushSize * 2, brushSize * 2,
                  null); // scaled image
         else if (cursorMode == FAUCET)
            g.drawImage(faucetMouse.getImage(), mouseX - (brushSize), mouseY - (brushSize), brushSize * 2,
                  brushSize * 2, null); // scaled image
         else
            g.drawImage(paintMouse.getImage(), mouseX - (brushSize), mouseY - (brushSize), brushSize * 2, brushSize * 2,
                  null); // scaled image
      }
   }
   ...
   public static void moveSnow() {
      for (int c = 0; c < snow[0].length; c++) {
         ...
         for (int r = snow.length - 1; r >= 0; r--) // bottom row moves first
         {
            ...
            if (r + 1 < snow.length && c - 1 >= 0) // belowLeft: color of snow (if any) of the spot below and left of
                                                   // the current snow
               belowLeft = snow[r + 1][c - 1];
            Color belowRight = null;
            if (r + 1 < snow.length && c + 1 < snow[0].length) // belowRight: color of snow (if any) of the spot below
                                                               // and right of the current snow
            ...
            if (type != null) {
               ...
               if (weight[weightIndex].contains("MEDIUM"))
                  ...
                  rand *= 8;
               if ((type.equals(Color.white)) || (typeBelow != null && typeBelow.equals(Color.white))
                     || (typeBelow != null && belowLeft != null && belowRight != null)) {
                  // snow stays put: its a wall, or there is a wall beneath it or other snow
                  // below, below left and below right
                  if (!type.equals(Color.white)) {
                     if (weight[weightIndex].contains("HEAVY")
                           || ((weight[weightIndex].contains("MEDIUM") && Math.random() < .5)
                                 || (windPushEast && right == null))) {
                        if (left == null && right == null) { // nothing left or right, so randomly move either left or
                                                             // right
                           snow[r][c] = null;
                  ...
                  if (belowLeft == null && belowRight == null) {
                     if (rand < .33 || windPushEast) {
                        ...
                        snow[r + 1][c] = type;
                  } else if (belowRight != null && belowLeft == null) { // nothing below, nothing below left, something
                                                                        // below right
                     if (rand < .33 && isValid(r + 1, c - 1))
                        snow[r + 1][c - 1] = type;
                     else if (isValid(r + 1, c))
                        snow[r + 1][c] = type;
                  } else if (belowLeft != null && belowRight == null) { // nothing below, nothing below right, something
                                                                        // below left
                     ...
                     snow[r + 1][c] = type;
               } else if (typeBelow != null && belowLeft == null && belowRight == null) { // if there is something below
                                                                                          // it but not below left and
                                                                                          // right, randomly move below
                                                                                          // left or below right
                  snow[r][c] = null;
         ...
         if (rain) {
            for (int i = 0; i < rainFreq; i++) // make snow fall from the top or from faucet
            {
               ...
               if (cursorMode == FAUCET) {
                  col = (int) (Math.random() * ((mouseX - brushSize) - (mouseX + brushSize) + 1)) + (mouseX - brushSize)
                        + brushSize * 2;
